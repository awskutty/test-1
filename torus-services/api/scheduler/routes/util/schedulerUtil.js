                    ...
                    if (objLogInfo.scheduleTheJob || !global.isLatestPlatformVersion) {
                        JSCHEDULE = schedule.scheduleJob({
                            ...
                            try {
                                ...
                                var checkJobStatusRes = await validateJobStatus(mDevCas, job_name, objLogInfo, job_description)
                                CL('I', ServiceName, 'Job status |' + checkJobStatusRes, objLogInfo);
                                if (checkJobStatusRes == "STARTED") {
                                    if (!thread_id) {
                                        thread_id = uuid.v4();
                                    }
                                    ...
                                    CL('I', ServiceName, 'Job | ' + job_description + ' status is stopped.Need to remove the job from cron ', objLogInfo);
...
function validateJobStatus(dbConn, job_name, objLogInfo, job_description) {
    try {
        return new Promise((resolve, reject) => {
            CL("I", ServiceName, "Validate the job status for job | " + job_description);
            var pcond = {
                'job_name': job_name
            }
            reqFXDBInstance.GetTableFromFXDB(dbConn, 'sch_jobs', [], pcond, objLogInfo, function (pError, pResult) {
                if (pError) {
                    CL("I", ServiceName, "Error occured while validate the job status for job | " + job_description + "Error  | " + pError);
                    resolve("STOPPED")
                } else {
                    if (pResult.rows[0].status == "STARTED") {
                        resolve("STARTED")
                    } else {
                        resolve("STOPPED")
                    }
        ...
        CL("I", ServiceName, "Exception occured while validate the job status for job | " + job_description + "Error  | " + error);
        resolve("STOPPED")
    }
}
    ...
    try {
        ...
        */
        if (params.objLogInfo && params.objLogInfo.doRedisPubSub && global.isLatestPlatformVersion) {
            ...
            reqRedisInstance.GetRedisConnectionwithIndex(4, async function (error, redisClient) {
                if (error) {
                ...
                await redisClient.publish('SCHEDULER_REDIS_PUBSUB', JSON.stringify(params.msgTobePublished))
...
function addJobLogCommon(mDevCas, objLogInfo, SCHJTL_ID, THREAD_ID, JOB_CODE, job_description, IS_REPEAT, API, METHOD_TYPE, METHOD_ARGS, JSCHEDULE, APP_ID, ROUTING_KEY, SCHEDULE_DET_OBJ, job_inc_id, start_time) {
    try {
        ...
        req_type_json.headers['from_scheduler'] = true;
        try {
            var jobNotificationReqObj = {
                jobInfo: jobInfo,
            ...
            for (let v = 0; v < threadCount; v++) {
                ...
                var schJobThreadRedisKey = APP_ID + '_' + JOB_CODE + "_" + job_description + '_' + "THREAD_COUNT" + "_" + v + '_' + JSON.stringify(dateTime);
                schJobThreadRedisKey = schJobThreadRedisKey.replace(/"/g, '');
                var schJobThreadRedisKeyTTL = 1; // 1Hr - For Job Thread Expiring Time in Reids 
                ...
                reqRedisInstance.GetRedisConnectionwithIndex(5, async function (error, redisClient) {
                    // redisClient.SETEX(schJobThreadRedisKey, schJobThreadRedisKeyTTL, JSON.stringify(schJobThreadRedisKeyValue));  
                    var result = await redisClient.set(schJobThreadRedisKey, '', { EX: schJobThreadRedisKeyTTL, NX: true });
                    if (!result) {
                        CL('I', ServiceName, ' Api already called from another instance ' + job_description, objLogInfo);
                    } else {
                        updateThreadLogCommon(mDevCas, objLogInfo, 'insert', job_inc_id, JOB_CODE, THREAD_ID, constants.THREADSTARTED, start_time, null, "", APP_ID, ROUTING_KEY, '', '', '', function (threadLogRes) {
                            ...
                            // req_type_json.body.url = '<test>/Communication/RetryMessages';
                            // req_type_json.url = '<test>/Communication/RetryMessages' 
                            req_type_json.headers['from_scheduler'] = true;
                            request(req_type_json, function (error, response, body) {
                                ...
                                if (error || response && response.statusCode !== 200 && response.statusCode !== 304) {
                                    ...
                                    updateThreadLogCommon(mDevCas, objLogInfo, 'update', SCHJTL_ID, JOB_CODE, THREAD_ID, constants.THREADABORTED, start_time, new Date(), error_msg, APP_ID, ROUTING_KEY, '', threadLogRes, '', function () { }); // Thread Completed
                                } else {
                                    // For Job Thread Stop
                                    ...
                                    updateThreadLogCommon(mDevCas, objLogInfo, 'update', SCHJTL_ID, JOB_CODE, THREAD_ID, constants.THREADCOMPLETED, start_time, new Date(), error_msg, APP_ID, ROUTING_KEY, notifymsg, threadLogRes, response.body, function () { }); // Thread Completed
                                    CL('I', ServiceName, '-----------------------\n', objLogInfo);
        ...
        updateJobsLogCommon(mDevCas, objLogInfo, 'update', constants.JOBABORTED, null, new Date(), "Exception:" + ex, JOB_CODE, job_description, APP_ID, ROUTING_KEY, SCHJTL_ID, THREAD_ID); // Job Completed
...
function updateJobsLogCommon(mDevCas, objLogInfo, query, job_status, start_time, end_time, error_msg, job_code, job_description, APP_ID, ROUTING_KEY, job_inc_id, jobthreadId) {
    try {
...
function updateThreadLogCommon(mDevCas, objLogInfo, query, code, job_code, thread_id, job_status, start_time, end_time, error_msg, APP_ID, ROUTING_KEY, result, threadloginserRes, apiResponse, updateThreadLogCommonCB) {
    try {
        jobHelper.AddThreadLogVariableParameter(mDevCas, query, objLogInfo, code, job_code, thread_id.toString(), job_status, start_time, end_time, error_msg, APP_ID, ROUTING_KEY, result, threadloginserRes, apiResponse,
            function (resAddThreadLog) {
                ...
                if (updateThreadLogCommonCB) {
                    updateThreadLogCommonCB(resAddThreadLog);
                }
            });
    } catch (error) {
...
function stopCronJob(pDevCas, objLogInfo, job_name, app_id, jobthreadId) {
    try {
        CL("I", ServiceName, "Stop Cron Job Called", objLogInfo);
        mDevCas = pDevCas;
        removeCronJob(objLogInfo, job_name, app_id);
        updateJobsLogCommon(mDevCas, objLogInfo, 'update', constants.JOBSTOPPED, null, new Date(), "", job_name, "", app_id, "", '', jobthreadId); // Job Completed
...
function callapi(obj) {
    try {
        ...
        for (let v = 0; v < threadCount; v++) {
            jobHelper.SendJobNotifications(jobNotificationReqObj);
            updateThreadLogCommon(mDevCas, objLogInfo, 'insert', job_inc_id, job_name, thread_id, constants.THREADSTARTED, onDemandJobStartTime, null, "", APP_ID, ROUTING_KEY, '', '', '', function (threadLogRes) {  // Thread Started
                ...
                request(req_type_json, function (error, response, body) {
                    ...
                    if (error || response && response.statusCode !== 200 && response.statusCode !== 304) {
                        ...
                        if (v == 0) {
                            ...
                            updateJobsLogCommon(mDevCas, objLogInfo, 'update', constants.JOBABORTED, onDemandJobStartTime, new Date(), "", job_name, job_description, APP_ID, ROUTING_KEY, job_inc_id, thread_id); // Job Started
                        }
                        updateThreadLogCommon(mDevCas, objLogInfo, 'update', job_inc_id, job_name, thread_id, constants.THREADABORTED, onDemandJobStartTime, new Date(), JSON.stringify(error), APP_ID, ROUTING_KEY, '', threadLogRes, '', function () { }); // Thread Started
                        ...
                        if (v == 0) {
                            jobHelper.UpdateJobStatus(mDevCas, objLogInfo, job_name, APP_ID, "STOPPED", function () { });
                            updateJobsLogCommon(mDevCas, objLogInfo, 'update', constants.JOBSTOPPED, onDemandJobStartTime, new Date(), "", job_name, job_description, APP_ID, ROUTING_KEY, job_inc_id, thread_id); // Job Started
                        }
                        updateThreadLogCommon(mDevCas, objLogInfo, 'update', job_inc_id, job_name, thread_id, constants.THREADSTOPPED, onDemandJobStartTime, new Date(), "", APP_ID, ROUTING_KEY, notifymsg, threadLogRes, response.body, function () { }); // Thread Started
                    }
                });
            });
        }
        ...
        jobHelper.UpdateJobStatus(mDevCas, objLogInfo, job_name, APP_ID, "ABORTED", function () { });
        setTimeout(function () {
            updateJobsLogCommon(mDevCas, objLogInfo, 'update', constants.JOBABORTED, onDemandJobStartTime, new Date(), JSON.stringify(error), job_name, job_description, APP_ID, ROUTING_KEY, job_inc_id, thread_id); // Job Started
            updateThreadLogCommon(mDevCas, objLogInfo, 'update', job_inc_id, job_name, thread_id, constants.THREADABORTED, onDemandJobStartTime, new Date(), JSON.stringify(error), APP_ID, ROUTING_KEY, '', '', '', function () { }); // Thread Started
        }, 2000);
    }
}

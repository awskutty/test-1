...
router.post('/GetAuditLogInfo', function callbackCpsignin(appRequest, appResponse) {
    ...
    reqLogInfo.AssignLogInfoDetail(appRequest, function callbackAssignLogInfoDetail(pLogInfo, pSessionInfo) {
        ...
        objResult.HeaderInfo = [
            ...
            { field: 'OLD_VALUE', header: 'Old Value' },
                            ...
                            if (error) {
                                ...
                                for (var i = 0; i < result.response.docs.length; i++) {
                                    ...
                                    if (isDate(resobj.OLD_VALUE)) {
                                        ...
                                        if (DB_Type == 'POSTGRES') {
											resobj.OLD_VALUE = reqMoment.parseZone(resobj.OLD_VALUE).utc(false).format("YYYY-MM-DD hh:mm:ss A")
                                        } else {
                                            resobj.OLD_VALUE = reqMoment.parseZone(resobj.OLD_VALUE).utc(false).format("YYYY-MM-DD hh:mm:ss A")  
                                        }
                                    ...
                                    if (isDate(resobj.NEW_VALUE)) {
                                        ...
                                        if (DB_Type == 'POSTGRES') {
											 resobj.NEW_VALUE = reqMoment.parseZone(resobj.NEW_VALUE).utc(false).format("YYYY-MM-DD hh:mm:ss A") 
                                        } else {
                                            resobj.NEW_VALUE = reqMoment.parseZone(resobj.NEW_VALUE).utc(false).format("YYYY-MM-DD hh:mm:ss A") 
                                        }

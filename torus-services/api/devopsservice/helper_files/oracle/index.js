...
async function CreateSchema(ClientParams) {
    ...
    var oracleEncryptedPassword = process.env.oracle_password;
    ClientParams.oracleDecryptedPassword = reqEncryptionInstance.DoDecrypt(oracleEncryptedPassword.toLowerCase());
    await ExecuteFileQuery(ClientParams, ClientParams.DatabaseFxPath, "ddl.sql");
    await ExecuteFileQuery(ClientParams, ClientParams.DatabaseFxPath, "dml.json");
...
async function ExecuteFileQuery(ClientParams, filepath, filename) {
    ...
    if (FileSystem.existsSync(strQryPath)) {
        ...
        if (filename == "ddl.sql" || filename == "dml.sql" || filename == "masterdata.sql" || filename == "dml.json") {
            //var keyspace =(filename == "masterdata.sql")?"maindb_public":"public";
            var keyspace = "maindb_public";
        ...
        if (ext == "json" || ext == "schema") {
            var strQueryJson = FileSystem.readFileSync(strQryPath).toString();
            if (filename == "dml.json") {
                strQueryJson = helper.StringReplaceAll(strQueryJson, "<database>", ClientParams.DB_NAME);
                strQueryJson = helper.StringReplaceAll(strQueryJson, "<password>", ClientParams.oracleDecryptedPassword);
            }
            var objQueryJson = [];
            if (strQueryJson != "") {
                ...
                if (fileName == "language_dictionary_source" || fileName == "language_dictionary_json") {
                    ...
                    for (var iqry = 0; iqry < objQueryJson.length; iqry++) {
                        ...
                        if (data.content) {
                            ...
                            if (fileName == "ddl_scripts" || fileName == "dml_scripts") {
                        ...
                        QryObj.query = helper.QueryTrim(QryObj.query);

                        if (QryObj.query.charAt(QryObj.query.length - 1) == ';' &&
                            QryObj.query.indexOf("CREATE TRIGGER") == -1 && QryObj.query.indexOf("CREATE OR REPLACE") == -1 &&
                            QryObj.query.indexOf("create trigger") == -1 && QryObj.query.indexOf("create or replace") == -1 &&
                            QryObj.query.indexOf("sp_dynamic_query_execute") == -1) {
                            QryObj.query = QryObj.query.substring(0, QryObj.query.length - 1);
                        }
                        if (QryObj.query.indexOf(";") > -1 &&
                            QryObj.query.indexOf("CREATE TRIGGER") == -1 && QryObj.query.indexOf("CREATE OR REPLACE") == -1 &&
                            QryObj.query.indexOf("create trigger") == -1 && QryObj.query.indexOf("create or replace") == -1 &&
                            QryObj.query.indexOf("sp_dynamic_query_execute") == -1) {
                            QryObj.query = QryObj.query.replace(/;/g, " ");
                        }
                        var result = {};
                        if (filename == "dml.json") {
                            if (QryObj.params.length == 0) {
                                result = await ExecuteDirectQuery(pSession, QryObj.query);
                            } else {
                                result = await ExecuteQuery(pSession, QryObj);
                            }
                        } else {
                            result = await ExecuteQuery(pSession, QryObj);
                        }
            ...
            for (var iqry = 0; iqry < splittedQuery.length; iqry++) {
                ...
                if (splittedQuery[iqry].toString().trim() == "" || splittedQuery[iqry].toString().trim() == " ") {
                    continue;
                }
                var qry = helper.QueryTrim(splittedQuery[iqry]);

                if (qry.charAt(qry.length - 1) == ';' &&
                    qry.indexOf("CREATE TRIGGER") == -1 && qry.indexOf("CREATE OR REPLACE") == -1 &&
                    qry.indexOf("create trigger") == -1 && qry.indexOf("create or replace") == -1 &&
                    qry.indexOf("sp_dynamic_query_execute") == -1) {
...
async function UpdateAppRoles(ClientParams) {
    ...
    if (DeployParams.SYSTEM_TYPES && DeployParams.SYSTEM_TYPES.length) {
        // prepare delete query for system_types
...
async function HandleFailedQueries(ClientParams) {
    console.log("HandleFailedQueries function started");
    var BatchQueries = [];
    ...
    if (FileSystem.existsSync(strCnfPath)) {
        ...
        if (strChangeInfo != "") {
            ...
            for (var t = 0; t < TruncateTable.length; t++) {
                var query = `delete from  ${TruncateTable[t]} where app_id = '${ClientParams.APP_ID}'`;
                    ...
                    if (resExtUser.rows.length > 0) {
                        ...
                        ExecuteQuery(pSession, FxItemObj).then((FxItems) => {
                            if (FxItems.qry_status == "SUCCESS") {
                                if (FxItems.rows.length > 0 && FxItems.rows[0].counter_value) {
...
async function ExecuteBulkQuery(ClientParams, pSession, BatchQueries, category) {
    console.log("ExecuteBulkQuery function started");
    try {
...
function ExecuteDirectQuery(pSession, Query) {
    console.log("ExecuteDirectQuery function started");
    return new Promise((resolve, reject) => {
...
async function GetDeployScriptItems(ClientParams) {
    console.log("GetDeployScriptItems function started");
    var pSession = undefined;
...
async function GetDeployDetails(ClientParams) {
    console.log("GetDeployDetails function started");
    var pSession = undefined;
    var pReturnVal = {
        "FirstTime": "Y",
        "DeployInfo": {}
...
async function QueryAction(ClientParams, QueryDetail, Action) {
    console.log("QueryAction function started");
    var pSession = undefined;
    ...
    if (Action == "DELETE") {
        ...
        var QueryObj = {
            "query": "delete from deployment_script_items where dsi_id='" + dsi_id + "'",
            "params": []
        };
        ...
        var QueryObj = {
            "query": `update deployment_script_items set query='${UpdateObj}' where dsi_id='${id}'`,
            "params": []
        };
            ...
            if (scriptItemResult.rows.length > 0) {
                ...
                for (var r = 0; r < scriptItemResult.rows.length; r++) {
                    ...
                    if (RetryResult.qry_status == "SUCCESS") {
                        var QueryObj = {
                            "query": "delete from deployment_script_items where dsi_id='" + data.dsi_id + "'",
                            "params": []
                        };
                        ...
                        var QueryObj = {
                            "query": `update deployment_script_items set query='${UpdateColumn}' where dsi_id='${data.dsi_id}'`,
                            "params": []
                        };
...
async function CheckDeployDetailStatus(ClientParams) {
    console.log("CheckDeployDetailStatus function started");
    var CurrentDateTime = new Date();
    var ClientId = ClientParams.CLIENT_ID;
    var EnvCode = ClientParams.ENV_CODE;
    var AppId = ClientParams.APP_ID;
...
async function CheckApplication(ClientParams) {
    console.log("CheckApplication function started");
    var ClientId = ClientParams.CLIENT_ID;
    var AppId = ClientParams.APP_ID;
    var pSession = await getDBInstance(ClientParams.headers, "clt_cas");
    var QueryObj = {
